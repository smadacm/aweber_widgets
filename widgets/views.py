...
# errors
# format is tuple(error_number, error_message, http_status)
#     where error_number is just an arbitrary int
...
def render_response(error: tuple[int, str, int] = ERR_A_OK,
                    error_message: str = '',
...
def create_or_update_widget(raw_body: str,
                            widget_id: typing.Optional[int] = None)\
        -> HttpResponse:
    """
    take the raw request body, and possibly a widget id,
    get or create the widget, apply the user's input, save it,
    and return a response object that can be returned by a view
    """
    error: typing.Optional[tuple[int, str, int]] = None
    error_message = ''

    # if there's no default for this, pycharm fusses
    parsed_body = {}
    ...
    return render_response(error=error or ERR_A_OK,
...
class Widgets(generic.View):
    # list all widgets
    def get(self, request: HttpRequest, *args: [any], **kwargs: dict)\
            -> HttpResponse:
        return render_response(widgets=models.Widget.objects.all())

    # create
    def put(self, request: HttpRequest, *args: [any], **kwargs: dict)\
            -> HttpResponse:
...
class ExistingWidget(generic.View):
    ...
    # delete
    def delete(self, request: HttpRequest, widget_id: int, *args: [any],
               **kwargs: dict) -> HttpResponse:
        error = ERR_A_OK
        widget = _get_widget_or_none(widget_id)
        if not widget:
